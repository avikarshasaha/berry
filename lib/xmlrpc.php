...
function & parse(&$request){
    if(defined('XMLRPC_DEBUG') and XMLRPC_DEBUG){
        self::debug('XMLRPC_parse', "<p>Received the following raw request:</p>" . self::show($request, 'print_r', true));
    }
    $data = &XML_unserialize(&$request);
    if(defined('XMLRPC_DEBUG') and XMLRPC_DEBUG){
        self::debug('XMLRPC_parse', "<p>Returning the following parsed request:</p>" . self::show($data, 'print_r', true));
    }
    return $data;
}

function & prepare($data, $type = NULL){
    if(is_array($data)){
...
function & adjustValue(&$current_node){
        ...
        if(isset($current_node['array'])){
            if(!is_array($current_node['array']['data'])){
                ...
                if(is_array($temp) and array_key_exists(0, $temp)){
                    ...
                    # $temp = array(&self::adjustValue(&$temp));
                }
            }
...
function request($site, $location, $methodName, $params = NULL, $user_agent = NULL){
    $site = explode(':', $site);
    ...
    if(defined('XMLRPC_DEBUG') and XMLRPC_DEBUG){
        self::debug('XMLRPC_request', "<p>Received the following parameter list to send:</p>" . self::show($params, 'print_r', true));
    }
    $conn = fsockopen ($site, $port); #open the connection
    if(!$conn){ #if the connection was not opened successfully
        if(defined('XMLRPC_DEBUG') and XMLRPC_DEBUG){
            self::debug('XMLRPC_request', "<p>Connection failed: Couldn't make the connection to $site.</p>");
        }
        ...
        if(defined('XMLRPC_DEBUG') and XMLRPC_DEBUG){
            self::debug('XMLRPC_request', "<p>Sent the following request:</p>\n\n" . self::show($headers . $data, 'print_r', true));
        }
        ...
        if(defined('XMLRPC_DEBUG') and XMLRPC_DEBUG){
            self::debug('XMLRPC_request', "<p>Received the following response:</p>\n\n" . self::show($response, 'print_r', true) . "<p>Which was serialized into the following data:</p>\n\n" . self::show($data, 'print_r', true));
        }
        if(isset($data['methodResponse']['fault'])){
            $return =  array(false, self::adjustValue(&$data['methodResponse']['fault']['value']));
            if(defined('XMLRPC_DEBUG') and XMLRPC_DEBUG){
                self::debug('XMLRPC_request', "<p>Returning:</p>\n\n" . self::show($return, 'var_dump', true));
            }
            return $return;
        }else{
...
function response($return_value, $server = NULL){
    ...
    if(defined('XMLRPC_DEBUG') and XMLRPC_DEBUG){
        self::debug('XMLRPC_response', "<p>Received the following data to return:</p>\n\n" . self::show($return_value, 'print_r', true));
    }
    ...
    if(defined('XMLRPC_DEBUG') and XMLRPC_DEBUG){
        self::debug('XMLRPC_response', "<p>Sent the following response:</p>\n\n" . self::show($return, 'print_r', true));
    }
    echo $return;
}

function error($faultCode, $faultString, $server = NULL){
    ...
    if(defined('XMLRPC_DEBUG') and XMLRPC_DEBUG){
        self::debug('XMLRPC_error', "<p>Sent the following error response:</p>\n\n" . self::show($return, 'print_r', true));
    }
    echo $return;
}

function convert_timestamp_to_iso8601($timestamp){
...
function convert_iso8601_to_timestamp($iso8601){
...
function debug($function_name, $debug_message){
