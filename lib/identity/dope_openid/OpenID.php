...
//*/
    ...
    function CURL_Request($url, $method="GET", $params = "") { // Remember, SSL MUST BE SUPPORTED
     ...
     function HTML2OpenIDServer($content) {
        ...
        // XRDS
        if (preg_match_all('/<URI>([^<]*)<\/URI>/', $content, $match)){
            $servers = array_merge($servers, $match[1]);
            $delegates = array_merge($delegates, $match[1]);
        }
    ...
    function GetRedirectURL(){
        ...
        if ($this->URLs['ns'])
            $params['openid.ns'] = urlencode($this->URLs['ns']);
